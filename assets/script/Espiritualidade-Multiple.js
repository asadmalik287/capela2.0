...
// console.log(mainData.espirtualData);
Object.keys(mainData.espirtualData).map((month) => {
    // console.log(month)
    Object.keys(mainData.espirtualData[month]).map((post) => {
        // console.log(mainData.espirtualData[month][post])
        if (month == n) {
            // console.log("sadsada");
            // mainData.espirtualData[month][post].date.split(' ')[0] <= new Date().getDate()

            if (Array.isArray(mainData.espirtualData[month][post]) == true) {
                mainData.espirtualData[month][post].map((subPost) => {
                ...
                if (
                    mainData.espirtualData[month][post].date.split(" ")[0] <=
                    new Date().getDate()
                ) {
                    // console.log(mainData.espirtualData[month][post]);
                    santoPostsArr.push(mainData.espirtualData[month][post]);
                }
            }
        } else {
            if (Array.isArray(mainData.espirtualData[month][post]) == true) {
